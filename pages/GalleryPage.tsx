  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0c0d0f] p-6 md:p-12 lg:p-20">
      <header className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-end mb-24 gap-8">
        <div>
          <h2 className="text-[11px] font-normal tracking-wider uppercase text-zinc-400 mb-6 opacity-60">04 — Work Process Gallery</h2>
          <h1 className="text-5xl md:text-7xl font-normal tracking-tight leading-none text-zinc-900 dark:text-zinc-50">Behind the Scenes</h1>
        </div>
        
        <div className="flex items-center gap-6">
          <div className="flex items-center bg-zinc-100 dark:bg-zinc-900 p-1.5 rounded-2xl">
            <button onClick={() => setColumns(1)} className={`px-4 py-2 rounded-xl text-[10px] font-normal uppercase tracking-wide transition-all ${columns === 1 ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-400'}`}>1 Col</button>
            <button onClick={() => setColumns(2)} className={`px-4 py-2 rounded-xl text-[10px] font-normal uppercase tracking-wide transition-all ${columns === 2 ? 'bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white shadow-sm' : 'text-zinc-400'}`}>2 Col</button>
          </div>
          <button onClick={() => setView('SITE')} className="px-8 py-3.5 bg-zinc-50 dark:bg-zinc-900 rounded-full border border-zinc-100 dark:border-zinc-800 font-normal text-[10px] uppercase tracking-wide hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all">Home</button>
        </div>
      </header>
          ...
          {galleryItems.map(item => (
            <div key={item.id} className="group animate-in fade-in slide-in-from-bottom-8 duration-1000 cursor-pointer" onClick={() => handleItemClick(item)}>
              <div className={`aspect-[16/10] bg-zinc-50 dark:bg-zinc-900 rounded-[32px] overflow-hidden mb-10 relative border border-zinc-50 dark:border-zinc-900`}>
                ...
                <div className="absolute top-8 right-8 px-4 py-2 bg-black/20 backdrop-blur-xl rounded-full text-[10px] font-normal text-white border border-white/20 uppercase tracking-wide">
                  {item.images.length} Photos
                </div>
              </div>
              <div className="px-4">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-2xl md:text-3xl font-normal tracking-tight text-zinc-900 dark:text-zinc-50">{item.projectTitle}</h3>
                  <span className="text-[10px] font-normal uppercase tracking-wide text-zinc-300 dark:text-zinc-700 pt-2">{item.period}</span>
                </div>
                <p className="text-zinc-600 dark:text-zinc-400 text-base md:text-lg font-normal leading-relaxed max-w-2xl line-clamp-2">{item.description}</p>
              </div>
            </div>
          ))}
