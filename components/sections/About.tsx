        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16 lg:gap-24">
          <div className="md:col-span-7 space-y-8 md:space-y-12">
            <div>
              ...
              <div className="space-y-6 md:space-y-10 text-lg md:text-2xl lg:text-3xl text-brand-black dark:text-zinc-200 leading-[1.3] md:leading-[1.4] font-normal antialiased">
                <p>{t.aboutMeText1}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4 md:gap-5">
              {SKILLS.map((skill, idx) => (
                <div key={idx} className="flex items-center gap-3 md:gap-4 group cursor-default">
                  <div className="w-1 md:w-1.5 h-1 md:h-1.5 rounded-full bg-zinc-200 dark:bg-zinc-700 group-hover:bg-brand-black transition-all duration-300"></div>
                  <span className="text-sm md:text-lg text-zinc-500 dark:text-zinc-400 group-hover:text-brand-black dark:group-hover:text-zinc-100 transition-colors duration-300 font-normal antialiased">
                    {skill}
                  </span>
                </div>
              ))}
